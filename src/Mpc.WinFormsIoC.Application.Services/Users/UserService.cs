    ...
    public class UserService : IUserService
    {
        ...
        public async Task<List<UserDto>> GetAllAsync()
        {
            ...
            var policy = await Policy
                ...
                .ExecuteAndCaptureAsync(async () => await _unitOfWork.UsersRepository.GetAllUsers().ConfigureAwait(false));  //GetByFilterAsync(1, 10).ConfigureAwait(false));
        ...
        public async Task UpdateAsync(UserDto user)
        {
            ...
            currentUser.PaisId = user.PaisId;
        ...
        public async Task DeleteAsync(UserDto user)
        {
            ...
            _unitOfWork.UsersRepository.Delete(currentUser);

            await _unitOfWork.SaveChangesAsync().ConfigureAwait(false);
        }
        
        public async Task<List<UserDto>> GetUsersByCountry(int countryId)
        {
            var policy = await Policy
                .Handle<Exception>()
                .WaitAndRetryAsync(3, _ => TimeSpan.FromSeconds(2))
                .ExecuteAndCaptureAsync(async () => await _unitOfWork.UsersRepository.GetByCountryAsync(countryId).ConfigureAwait(false));
